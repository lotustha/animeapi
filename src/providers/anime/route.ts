  ...
  .get(
    ...
    () => ({
      ...
      endpoints: {
        ...
        toonstream: [
          "GET /anime/toonstream/search/:query?page=      → Paginated search",
          "GET /anime/toonstream/suggestions/:query       → Search suggestions",
          "GET /anime/toonstream/spotlight                → Spotlight titles",
          "GET /anime/toonstream/recent-episodes          → Latest episodes rail",
          "GET /anime/toonstream/movies?page=             → Browse movies",
          "GET /anime/toonstream/tv?page=                 → Browse series",
          "GET /anime/toonstream/genres                   → Available genres (site categories)",
          "GET /anime/toonstream/genre/:genre?page=       → Browse by genre",
          "GET /anime/toonstream/info/:id                 → Full details + episodes",
          "GET /anime/toonstream/watch/:episodeId         → Stream sources (query: type=softsub|dub|hardsub)",
          "GET /anime/toonstream/servers/:episodeId       → Episode servers (query: type)",
          "    ↳ Same node output as /anime/animekai. ids are movie$<slug> / series$<slug>;",
          "      episode ids append $ep=<season>x<episode> (movies use $ep=1).",
          "    ↳ as-cdn21 (Multi Audio), rubystm (Ruby) and vidmoly resolve to direct streams",
          "      (HLS ones carry a qualities[] ladder); the rest (gdmirrorbot, cloudy.upns,",
          "      abyssplayer, blakiteapi, emturbovid) stay iframe-only. Every server is",
          "      multi-audio, so all `type` values return the same servers.",
          "GET /anime/toonstream/m3u8-proxy?url=&headers= → HLS proxy",
          "GET /anime/toonstream/ts-segment?url=&headers= → Segment proxy",
          "GET /anime/toonstream/mp4-proxy?url=&headers=  → MP4 proxy",
          "GET /anime/toonstream/fetch?url=&headers=      → Generic media proxy",
        ],
