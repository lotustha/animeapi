  ...
  .get(
    ...
    () => ({
      ...
      endpoints: {
        ...
        toonstream: [
          "GET /anime/toonstream/home                     → Home rails + latest episodes",
          "GET /anime/toonstream/search/:query/:page?     → Search (not paginated upstream: one page)",
          "GET /anime/toonstream/movies/:page?            → Browse movies",
          "GET /anime/toonstream/movies/info/:slug        → Movie details",
          "GET /anime/toonstream/movies/sources/:slug     → Movie embeds + direct sources",
          "GET /anime/toonstream/series/:page?            → Browse series",
          "GET /anime/toonstream/series/info/:slug        → Series details + seasons/episodes",
          "GET /anime/toonstream/episode/sources/:slug    → Episode sources (query: season, episode)",
          "    ↳ `embeds[]` lists every player host; `sources[]` holds the ones that reduce to a",
          "      direct stream — as-cdn21 (Multi Audio), rubystm (Ruby) and vidmoly. The rest",
          "      (gdmirrorbot, cloudy.upns, abyssplayer, blakiteapi, emturbovid) are iframe-only.",
          "    ↳ vidmoly's stream is network-bound; use its `proxiedUrl`, not `url`.",
